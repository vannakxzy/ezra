...
part 'result_discussion_event.dart';
part 'result_discussion_state.dart';
part 'result_discussion_bloc.freezed.dart';

@Injectable()
class ResultDiscussionBloc
    extends BaseBloc<ResultDiscussionEvent, ResultDiscussionState> {
  ResultDiscussionBloc(
  ...
  FutureOr<void> _clickShareQuestion(
      ClickShareQuestion event, Emitter<ResultDiscussionState> emit) async {
    await shareQuestion(event.question);
  }

  FutureOr<void> _getResultSearch(
      SearchQuestionEvent event, Emitter<ResultDiscussionState> emit) async {
    final recentSearch = await loadDiscussion();
  ...
  FutureOr<void> _clickSaveQuestion(
      ClickSaveQuestion event, Emitter<ResultDiscussionState> emit) async {
  ...
  FutureOr<void> _clickQuestion(
      ClickQuestionEvetn event, Emitter<ResultDiscussionState> emit) async {
    appRoute.push(AppRouteInfo.questionDetail(
        id: event.id, questionEntity: event.question));
    storeDisscuss(event.question);
  }

  FutureOr<void> _clickHide(
      ClickHideEvent event, Emitter<ResultDiscussionState> emit) async {
    await runAppCatching(
      () async {
  ...
  FutureOr<void> _clickUnHide(
      ClickUnHideEvent event, Emitter<ResultDiscussionState> emit) async {
    await runAppCatching(
      () async {
        final question = [...state.questions];
        final currentQuestion = question[event.index];
  ...
  FutureOr<void> _clickDoubleTap(
      DoubleTapEvent event, Emitter<ResultDiscussionState> emit) async {
    await runAppCatching(
      () async {
        List<QuestionEntity> questions = [...state.questions];
        final currentQuestion = questions[event.index];
  ...
  FutureOr<void> _clickLike(
      ClickLikeEvent event, Emitter<ResultDiscussionState> emit) async {
    await runAppCatching(
      () async {
        List<QuestionEntity> questions = [...state.questions];
        final currentQuestion = questions[event.index];
  ...
  FutureOr<void> _onRefreshpage(
      OnRefreshPageEvent event, Emitter<ResultDiscussionState> emit) async {
    await runAppCatching(
      () async {
  ...
  Future<void> storeDisscuss(QuestionEntity question) async {
    final jsonString = LocalStorage.getStringValue(
        SharedPreferenceKeys.recentSearchDiscussion);
    ...
    await LocalStorage.storeData(
      key: SharedPreferenceKeys.recentSearchDiscussion,
      value: jsonEncode(jsonList),
    );
  }

  FutureOr<void> _removeIndex(
      RemoveIndex event, Emitter<ResultDiscussionState> emit) {
  ...
  FutureOr<void> _removeAll(
      RemoveAll event, Emitter<ResultDiscussionState> emit) async {
    await LocalStorage.remove(SharedPreferenceKeys.recentSearchDiscussion);
    emit(state.copyWith(recentSearch: []));
  }

  Future<List<QuestionEntity>> loadDiscussion() async {
    try {
      final jsonString = LocalStorage.getStringValue(
          SharedPreferenceKeys.recentSearchDiscussion);
      final List<dynamic> jsonList = jsonDecode(jsonString);
      List<QuestionEntity> discussion =
          jsonList.map((e) => QuestionEntity.fromJson(e)).toList();
      debugPrint("recent answer  ${discussion.length}");
      return discussion;
    } catch (value) {
