...
part 'comment_in_discussion_event.dart';
part 'comment_in_discussion_state.dart';
part 'comment_in_discussion_bloc.freezed.dart';

@Injectable()
class CommentInDiscussionBloc
    extends BaseBloc<CommentInDiscussionEvent, CommentInDiscussionState> {
  CommentInDiscussionBloc(
      ...
      : super(const CommentInDiscussionState()) {
  ...
  FutureOr<void> _clickLike(
      ClickLike event, Emitter<CommentInDiscussionState> emit) async {
  ...
  FutureOr<void> _clickEdit(
      ClickEditEvent event, Emitter<CommentInDiscussionState> emit) async {
  ...
  FutureOr<void> _update(
      ClickUpdateEvent event, Emitter<CommentInDiscussionState> emit) async {
    await runAppCatching(
      () async {
  ...
  FutureOr<void> _cancelEdit(
      ClickCancelEditEvent event, Emitter<CommentInDiscussionState> emit) {
    emit(state.copyWith(
        oldMessage: "",
  ...
  FutureOr<void> _clickDeleteComment(
      ClickDeleteEvent event, Emitter<CommentInDiscussionState> emit) async {
    await runAppCatching(
      () async {
  ...
  FutureOr<void> _getcommentInAnswer(
      GetCommentEvent event, Emitter<CommentInDiscussionState> emit) async {
    await runAppCatching(
      () async {
        emit(state.copyWith(isloading: true));
        final commentResopose = await _getCommentInAnswer.excecute(
            GetCommentAnswerInput(
                page: state.page, answer_id: event.discussionId));
  ...
  FutureOr<void> _refresPage(
      RefresPage event, Emitter<CommentInDiscussionState> emit) async {
    await runAppCatching(
      () async {
  ...
  FutureOr<void> _desChange(
      DesChangedEvent event, Emitter<CommentInDiscussionState> emit) async {
    emit(state.copyWith(message: event.text));
    _enableComment(emit);
  }

  FutureOr<void> _createComment(ClickCreateEventCommentEvent event,
      Emitter<CommentInDiscussionState> emit) async {
    try {
      List<CommentEntity> comment = [...state.comments];
  ...
  void _enableComment(Emitter<CommentInDiscussionState> emit) {
