...
import '../../../../question_detail/domain/usecase/delete_question_usecase.dart';
import '../../../../question_detail/domain/usecase/get_question_by_id.dart';
...
part 'discussion_detail_bloc.freezed.dart';

@Injectable()
class DiscussionDetailBloc
    extends BaseBloc<DiscussionDetailEvent, DiscussionDetailState> {
  ...
  Future<void> _clickReplyComment(
      ClickReplyComment event, Emitter<DiscussionDetailState> emit) async {
    emit(state.copyWith(
        isComment: true,
        message: '',
  ...
  Future<void> _clickUpdateComment(
      ClickUpdateComment event, Emitter<DiscussionDetailState> emit) async {
  ...
  Future<void> _clickCancelReply(
      ClickCancelReply event, Emitter<DiscussionDetailState> emit) async {
    emit(state.copyWith(
        isComment: false,
        message: '',
        replyTo: '',
  ...
  FutureOr<void> _clickEditComment(
      ClickEditComment event, Emitter<DiscussionDetailState> emit) async {
    final answer = [...state.answer];
  ...
  FutureOr<void> _createComment(
      CreateComment event, Emitter<DiscussionDetailState> emit) async {
    await runAppCatching(
      () async {
  ...
  FutureOr<void> _clickLikeComment(
      ClickLlikeComment event, Emitter<DiscussionDetailState> emit) async {
    final answer = [...state.answer];
    AnswertEntity currentAnswer = answer[event.answerIndex];
    List<CommentEntity> comments = [...currentAnswer.comments];
  ...
  FutureOr<void> _clickDeleteComment(
      ClickDeleteComment event, Emitter<DiscussionDetailState> emit) async {
  ...
  FutureOr<void> _clickGetComment(
      ClickGetComment event, Emitter<DiscussionDetailState> emit) async {
    final answer = [...state.answer];
  ...
  FutureOr<void> _initPage(
      InitPage event, Emitter<DiscussionDetailState> emit) {
    if (event.discussion == QuestionEntity()) {
      add(GetDiscussionByIdEvent(event.discussionId));
      add(GetAnswerInDiscussionEvent(event.discussionId));
    } else {
      emit(state.copyWith(discussion: event.discussion));
      add(GetAnswerInDiscussionEvent(event.discussion.id));
    }
  }

  FutureOr<void> _clickLikeDisscussion(
      ClickLikeDiscussion event, Emitter<DiscussionDetailState> emit) async {
    debugPrint("sdfdsf");

    QuestionEntity currentDiscussion = state.discussion;
    ...
    if (currentDiscussion.is_like) {
      ...
      emit(state.copyWith(discussion: currentDiscussion));
  ...
  FutureOr<void> _getQuestionById(
      GetDiscussionByIdEvent event, Emitter<DiscussionDetailState> emit) async {
    debugPrint("fasfsaf");
    await runAppCatching(
      () async {
        emit(state.copyWith(isloading: true));
        final discussion = await _getQuestionByIdUseCase.excecute(event.id);
        emit(state.copyWith(isloading: false, discussion: discussion));
      },
      onError: (error) async {
  ...
  FutureOr<void> _scrollDirectionUpdateEvent(
      ScrollDirectionUpdate event, Emitter<DiscussionDetailState> emit) {
    emit(state.copyWith(isScrollingDown: event.value));
  }

  FutureOr<void> _listionScroll(
      ListionScroll event, Emitter<DiscussionDetailState> emit) {
    if (event.value > 100) {
  ...
  FutureOr<void> _clcikPostDiscussion(
      ClcikPostDiscussion event, Emitter<DiscussionDetailState> emit) async {
    final answer = [...state.answer];
    QuestionEntity discussion = state.discussion;
    discussion =
        discussion.copyWith(amountAnswers: state.discussion.amountAnswers + 1);
    answer.insert(
      0,
      AnswertEntity(
          date: t.common.justNow,
  ...
  FutureOr<void> _clickCancelEdit(
      ClickCancelEdit event, Emitter<DiscussionDetailState> emit) {
    emit(state.copyWith(
      file: null,
      oldMessage: "",
      messageController: TextEditingController(text: ""),
  ...
  void _enablePostDiscussion(Emitter<DiscussionDetailState> emit) {
  ...
  FutureOr<void> _clickBlock(
      ClickBlockUserEvent event, Emitter<DiscussionDetailState> emit) async {
    await runAppCatching(
      () async {
  ...
  FutureOr<void> _clickUpdateAnswer(
      ClickUpdateAnswer event, Emitter<DiscussionDetailState> emit) async {
    await runAppCatching(
      () async {
  ...
  FutureOr<void> _updateQuestion(
      ClickUpdateQuestion event, Emitter<DiscussionDetailState> emit) async {
    emit(state.copyWith(discussion: event.discussion));
    // debugPrint()
  }

  FutureOr<void> _pickImage(
      ClickPickImageEvent event, Emitter<DiscussionDetailState> emit) async {
    emit(state.copyWith(file: event.file));
    _enablePostDiscussion(emit);
  }

  FutureOr<void> _clearImage(
      ClickClearImageEvent event, Emitter<DiscussionDetailState> emit) async {
    emit(state.copyWith(file: null));
    _enablePostDiscussion(emit);
  }

  FutureOr<void> _desChanged(
      MessageChangedEvent event, Emitter<DiscussionDetailState> emit) {
    emit(state.copyWith(message: event.value));
    _enablePostDiscussion(emit);
  }

  FutureOr<void> _clickEditAnswer(
      ClickEditAnswer event, Emitter<DiscussionDetailState> emit) async {
  ...
  FutureOr<void> _deleteAnswer(
      ClickDeleteAnswer event, Emitter<DiscussionDetailState> emit) async {
    await runAppCatching(
      () async {
        // appRoute.pop();
        final discussion = state.discussion
            .copyWith(amountAnswers: state.discussion.amountAnswers - 1);
        final answer = [...state.answer];
        final answerId = answer[event.index].id;
        answer.removeAt(event.index);
        emit(state.copyWith(answer: answer, discussion: discussion));
  ...
  FutureOr<void> _clickDeleteDiscussion(ClickDeleteDiscusstionEvent event,
      Emitter<DiscussionDetailState> emit) async {
    await runAppCatching(
      () async {
        await _deleteQuestionUsecase.excecute(event.questionId);
        appRoute.showInfoSnackBar(t.discussion.deleteSuccess);
      },
    );
  }
  ...
  //     ClickHideEvent event, Emitter<DiscussionDetailState> emit) async {
  ...
  FutureOr<void> _clickDownloadImageDiscussion(ClickDownloadPhoroEvent event,
      Emitter<DiscussionDetailState> emit) async {
  ...
  FutureOr<void> _ClickLikeAnswer(
      ClickLikeAnswerEvent event, Emitter<DiscussionDetailState> emit) async {
    final answer = [...state.answer];
  ...
  FutureOr<void> _clickShare(
      ClickShare event, Emitter<DiscussionDetailState> emti) async {
    await shareDiscussion(event.discussion);
  }
}
