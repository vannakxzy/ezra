...
import '../../domain/usecase/update_question_usecase.dart';
...
@Injectable()
class EditQuestionBloc extends BaseBloc<EditQuestionEvent, EditQuestionState> {
  EditQuestionBloc(
      this._getTagUseCase, this._createTagUseCase, this._updateQuestionUsecase)
      : super(const EditQuestionState()) {
  ...
  Future<void> _initPageEvent(
      InitPageEvent event, Emitter<EditQuestionState> emit) async {
    emit(state.copyWith(
  ...
  Future<void> _listenerFocusNode(
      ListenerFocusNodeEvent event, Emitter<EditQuestionState> emit) async {
    emit(state.copyWith(isFocus: event.focus));
    debugPrint("isFocus ${state.isFocus}");
  }

  FutureOr<void> _cropImage(
      ClickCropEvent event, Emitter<EditQuestionState> emit) async {
    final newImage = await cropImage(state.image!);
    emit(state.copyWith(image: newImage));
  }

  FutureOr<void> _audienceChange(
      AuienceChanged event, Emitter<EditQuestionState> emit) async {
    emit(state.copyWith(
  ...
  FutureOr<void> _createTag(
      ClickCreateTagEvent event, Emitter<EditQuestionState> emit) async {
  ...
  FutureOr<void> _validationPost(Emitter<EditQuestionState> emit) {
    try {
  ...
  FutureOr<void> _updateQuestion(
      ClickUpdateQuestionEvent event, Emitter<EditQuestionState> emit) async {
  ...
  FutureOr<void> _clearImage(
      ClickClearImageEvent event, Emitter<EditQuestionState> emit) {
    emit(state.copyWith(
        image: null,
        urlImage: '',
  ...
  FutureOr<void> _getTag(
      TagChangedEvent event, Emitter<EditQuestionState> emit) async {
  ...
  FutureOr<void> _clickSelectTag(
      ClickSelectTagEvent event, Emitter<EditQuestionState> emit) {
    try {
  ...
  FutureOr<void> _clickRemoveTag(
      ClickRemoveTagEvent event, Emitter<EditQuestionState> emit) {
  ...
  FutureOr<void> _pickImageCamera(
      PickImageCameraEvent event, Emitter<EditQuestionState> emit) async {
  ...
  FutureOr<void> _pickImageGallery(
      PickImageGalleryEvent event, Emitter<EditQuestionState> emit) async {
  ...
  FutureOr<void> _titleChanged(
      TitleChangedEvent event, Emitter<EditQuestionState> emit) async {
  ...
  FutureOr<void> _descriptionChanged(
      DescriptionChangedEvent event, Emitter<EditQuestionState> emit) async {
    emit(state.copyWith(
